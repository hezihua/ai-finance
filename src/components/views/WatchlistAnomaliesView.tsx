      ...
      <div className="flex flex-wrap items-end justify-between gap-3">
        ...
        <div className="flex flex-wrap gap-2">
          <input
            ...
            placeholder="添加自选…"
            className="rounded-lg border border-white/10 bg-[#1a1f28] px-3 py-2 text-sm outline-none"
          />
          <button
            onClick={() => void addByQuery()}
            className="inline-flex items-center gap-1 rounded-lg bg-emerald-500/15 px-3 py-2 text-sm text-emerald-400"
          >
            ...
            <table className="w-full min-w-[880px] text-left text-sm">
              ...
              <tbody>
                {rows.map(({ snap, name, anomalies: ans }) => (
                  <tr key={snap.thscode} className="border-b border-white/5 align-top">
                        ...
                        <ul className="space-y-1">
                          {ans.map((a, i) => (
                            <li key={i} className="text-xs leading-relaxed">
                              <span className="mr-2 rounded bg-white/5 px-1.5 py-0.5 text-emerald-300">
                                {a.tag_name || "异动"}
                              </span>
                              {a.analysis_content}
                            </li>
