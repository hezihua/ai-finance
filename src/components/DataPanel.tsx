...
export default function DataPanel() {
  ...
  if (!activeModuleId || !module) {
    return (
      <div className="flex h-full flex-col items-center justify-center bg-[#141210] px-6 text-center text-white/40">
  ...
  return (
    <div className="flex h-full flex-col border-r border-white/5 bg-[#141210] text-[#e8e4e0]">
      ...
      <div className="dark-scrollbar flex-1 space-y-5 overflow-y-auto px-4 py-4">
        <section className="rounded-xl border border-white/5 bg-[#1a1614] p-4">
          <p className="text-xs font-medium text-white/50">数据源</p>
          <p className="mt-1 break-all font-mono text-[11px] text-emerald-400/90">
            {module.endpoint}
          </p>
        ...
        {needsStock && (
          <section className="space-y-3">
            ...
            <div className="flex gap-2">
              <div className="relative flex-1">
                ...
                <input
                  ...
                  placeholder="股票名称或代码"
                  className="w-full rounded-lg border border-white/5 bg-[#231f1b] py-2.5 pl-9 pr-3 text-sm text-white/90 placeholder:text-white/30 focus:border-emerald-500/30 focus:outline-none"
                />
              </div>
              <button
                onClick={() => void searchSymbols()}
                disabled={loading}
                className="rounded-lg bg-emerald-500/15 px-3 text-sm text-emerald-400 transition hover:bg-emerald-500/20 disabled:opacity-50"
              >
                ...
                {results.map((item) => (
                  <li key={item.thscode}>
                    <button
                      ...
                      className="flex w-full items-center justify-between px-3 py-2.5 text-left text-sm transition hover:bg-[#231f1b]"
                    >
                      <span>
            ...
            <div className="rounded-lg border border-white/5 bg-[#231f1b] px-3 py-3">
        ...
        {activeModuleId === "market-heat" && (
          <section className="rounded-xl border border-white/5 bg-[#1a1614] p-4 text-xs leading-relaxed text-white/45">
