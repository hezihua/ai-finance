          ...
          <table className="w-full min-w-[900px] text-left text-sm">
            ...
            <tbody>
              {filtered.slice(0, 80).map((item) => (
                <tr key={item.thscode} className="border-b border-white/[0.07]">
        ...
        <Panel title="游资席位" subtitle="按买入额排序，仅展示前 20">
          <div className="grid gap-3 md:grid-cols-2">
            {hotMoney.slice(0, 20).map((hm) => (
              <div key={hm.name} className="rounded-xl border border-white/[0.07] bg-white/[0.02] p-3">
                <div className="flex items-center justify-between gap-2">
                  <p className="font-medium text-white/90">{hm.name}</p>
                  <p className="text-sm tabular-nums text-[#8eb6ff]">{formatMoney(hm.buying)}</p>
                </div>
